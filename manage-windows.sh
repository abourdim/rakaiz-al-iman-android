#!/usr/bin/env bash
# ============================================================================
#  Rakaiz al-Iman Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/43-rakaiz-al-iman}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.rakaizaliman}"
AVD_NAME="${AVD_NAME:-rakaiz_al_iman_test}"
...
cmd_test() {
  ...
  local out="/tmp/rakaiz-al-iman-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Rakaiz al-Iman Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
