/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Rakaiz al-Iman
...
(function() {
  ...
  function extractCards(panel) {
    ...
    if (tabName === 'about') {
      panel.querySelectorAll('.about-card, .about-disclaimer, .about-author, .about-section').forEach(el => {
      ...
      panel.querySelectorAll('.pillar-card').forEach(el => {
        ...
        const t = el.querySelector('.pillar-title');
        const d = el.querySelector('.pillar-desc');
        ...
        cards.push({ el, text: cleanText(text), type: 'pillar' });
      });
    } else if (tabName === 'heart') {
      panel.querySelectorAll('.insight-card').forEach(el => {
        const text = cleanText(el.textContent);
        cards.push({ el, text, type: 'insight' });
      });
    } else if (tabName === 'build') {
      // Build tab is visual, skip
  ...
  function cleanText(text) {
    ...
    if (l === 'fr') {
      clean = clean
        ...
        .replace(/Rakaiz al-Iman/gi, 'Rakaize al Imane')
    ...
    if (l === 'en') {
      clean = clean
        ...
        .replace(/Rakaiz al-Iman/gi, 'Rah-kah-iz al Ee-man')
  ...
  function readCurrentCard() {
    ...
    // Open pillar cards so content is visible
    if (card.type === 'pillar' && card.el && !card.el.classList.contains('open')) {
  ...
  function readDuoTranslation(card, onEnd) {
    ...
    // Try to get structured data for pillars
    const pillarId = card.el ? card.el.id : '';
    const pMatch = pillarId.match(/pillar-(\d+)/);
    if (pMatch && typeof PILLARS !== 'undefined') {
      const p = PILLARS.find(function(x) { return x.id === parseInt(pMatch[1]); });
      if (p && p[duoLang]) {
        duoText = p[duoLang].title + '. ' + p[duoLang].mind + '. ' + p[duoLang].heart;
      }
    }

    // Try insight data
    if (!duoText && card.type === 'insight' && typeof INSIGHTS !== 'undefined') {
      const insightCards = Array.from(card.el.parentNode.querySelectorAll('.insight-card'));
      const idx = insightCards.indexOf(card.el);
      if (idx >= 0 && INSIGHTS[idx] && INSIGHTS[idx][duoLang]) {
        const a = INSIGHTS[idx][duoLang];
        duoText = (a.title || '') + '. ' + (a.desc || '') + '. ' + (a.mind || '');
      }
    }
  ...
  function setupMediaSession() {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: l === 'ar' ? 'ركائز الإيمان' : l === 'fr' ? 'Piliers de la Foi' : 'Pillars of Faith',
  ...
  function onSpeedChange(val) {
    ...
    localStorage.setItem('ri-narrator-speed', STATE.speed);
  }
  ...
  function onPitchChange(val) {
    ...
    localStorage.setItem('ri-narrator-pitch', STATE.pitch);
  }
  ...
  function onKaraokeToggle(checked) {
    ...
    localStorage.setItem('ri-narrator-karaoke', checked);
  }

  function onAutoScrollToggle(checked) {
    STATE.autoScroll = checked;
    localStorage.setItem('ri-narrator-autoscroll', checked);
  }

  function onDuoToggle(checked) {
    STATE.duoReading = checked;
    localStorage.setItem('ri-narrator-duo', checked);
  }
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('ri-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('ri-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('ri-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('ri-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('ri-narrator-duo') === 'true';
  }
  ...
  function injectSpeakButtons() {
    ...
    var selectors = [
      '.about-card', '.about-disclaimer', '.about-author', '.about-section',
      '.pillar-card', '.insight-card', '.daily-card'
    ];
  ...
  if (origRenderPillars) {
    window.renderPillars = function() { origRenderPillars(); setTimeout(injectSpeakButtons, 100); };
  }
  var origRenderHeart = window.renderHeart;
  if (origRenderHeart) {
    window.renderHeart = function() { origRenderHeart(); setTimeout(injectSpeakButtons, 100); };
  }
